...
export function formatTopicDisplay(state, regenInterval = 10) {
    if (!state) {
...
function getCircleForProgress(count, nextAt, hasTopicAlready) {
...
function getConversationalMessage(circle, count, hasTopicAlready, remaining) {
    // Generating state
    if (circle === '●') {
    ...
    if (!hasTopicAlready) {
        if (count === 0)
            return 'Just getting started...';
        if (count === 1)
            return 'Getting to know your session...';
        if (remaining === 1)
            return 'Almost ready for a topic...';
        return 'Building up context...';
    }
    // Topic refinement (has topic already)
    if (remaining === 1)
        return 'Almost time to update...';
    if (remaining <= 3)
        return 'Nearly ready to refine...';
